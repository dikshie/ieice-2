...
Figure \ref{fig:businessrelationship} shows how contents in Internet are delivered to end-users.  
Content producers such as movie companies send their movies to content provider for example: Netflix and Hulu.
Content providers then deliver the movies using CDN or they also can use their upstream provider.
Depending on the peering policy, CDN can reach to customer directly via ISP or via IXP then reach ISP or goes to transit networks then reach ISP.   
In the current modern Internet topology, content providers and service providers may actually be a single entity.
...
The CDN proxies (Level-0) are maintained by ISP. CDN nodes (Level-1) can exchange contents with other CDN nodes owned by other CDN companies. P2P nodes are ISP customers running a P2P software.}
\label{fig:twotier}
\vspace{-2mm}
\end{figure} 
