...
\begin{equation}\label{eq:isppayoff}
	\pi_{isp}^{(s)} = N P^{(s)} - \tau^{(s)} - C_i^{(s)} + C^{(s)}_{p}
\end{equation}
Where $C_p$ is penalty cost for user. 
\end{theorem}
In next subsection, we will explain about penalty cost.
...
User get penalty cost from ISP if user turns off his or her home gateway if user accept ISP strategy $s_2$.
...
\begin{equation}\label{la:isppricereduction}
...
Now we look at user level strategy for ISP strategy $s_1$.  
Since there is only one user strategy for ISP strategy $s_1$ we can not compare to other.  
Next, we analyze the case user strategy when ISP offer strategy $s_2$. 
In this case: 
\begin{equation}
 \pi^{(\hat{s_a}|s_2)}_{u} = \pi^{(\hat{s_d}|s_2)}_{u}
\end{equation}
by simplification using Eq.\ref{la:isppricereduction} we can get:
\begin{equation}
C^{(s_2)}_p = \frac{\mu}{N} ( \tau^{(s_1)} - \tau^{(s_2)} - C^{(s_2)}_i )
\end{equation}
User accepts ${(\hat{s_a}|s_2)}$ if expected penalty less than cost reduction.
...
\begin{equation}\label{la:ISPpayoff}
...
\begin{eqnarray}\label{la:PS_2}
	\nonumber P^{(s_2)} &=& P^{(s_1)} - \theta^{(s_2)} \\
	P^{(s_2)} &=& P^{(s_1)} - \frac{\gamma}{N} (\tau^{(s_1)} - \tau^{(s_2)} - C^{(s_2)}_i )
\end{eqnarray}
Finally if we submitted Eq.\ref{la:PS_2} to Eq.\ref{la:ISPpayoff}, then we can get:
\begin{equation}
  \tau^{(s_2)}	- C^{(s_2)}_{i} = \tau^{(s_1)}
\end{equation}
The strategy restrictions as follows:
\begin{itemize}
	\item Choose strategy $s=s_2$ if $ \tau^{(s_2)}	- C^{(s_2)}_{i} < \tau^{(s_1)} $
	\item Choose strategy $s=s_1$ if $ \tau^{(s_2)}	- C^{(s_2)}_{i} > \tau^{(s_1)} $ 
\end{itemize}
...
\end{center}
\caption{Delta between with $U_s$ variation and $\gamma$ variation.}
\label{fig:discount}
\vspace{-2mm}
 \end{figure}
...
\begin{itemize}
	...
	\item We assume users never turn off their home gateway.
\end{itemize}

Figure \ref{fig:discount} shows subscription fee reduction that ISP can give to users.
