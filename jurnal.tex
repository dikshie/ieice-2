...
\begin{itemize}
	...
	\item $s_2 = (CDN-P2P, P^{s_2})$ : the ISP decides recommends peer-assited and offers a subscription fee $P^{s_2} \le P^{s_1}$.
\end{itemize}
...
\begin{itemize}
	...
	\item $\alpha^{(s)}$ represent the expected reduction factor for strategy $s$ such that: $\alpha^{(s)} \in [0,1]$ for $s \in \{s2\}$;  $\alpha^{(s_1)} = 1$.
...
More specifically refer to Fig.\ref{fig:twotier2} ISP required to setup their Level0 or CDN proxy and also provide different home gateway for users.
