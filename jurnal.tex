...
In this paper we study, the incentive for ISP who adopt peer assisted CDN for streaming purpose and shows the advantages to adopt peer assisted CDN. 
...
In this system, we define a seeder is a node or a peer that receive stream from CDN proxy and a leecher is a node or a peer that receive stream from a seeder or other a leecher.  
...
Figure \ref{fig:U_p_1000} shows numerical results of $U_p$ with churn where total number of peers in the system is $N=1000$ and Fig.\ref{fig:U_p_10000} shows numerical results of $U_p$ with churn where total numbers of peers in the system is $N=10000$
In churn case, the arrival and departure of peers following poisson process. 
When peers arrive CDN proxy try to stabilized the system using simple admission that we explain in sec.\ref{sec:stablesystemwithchurn}.
As we can see that the min capacity is increasing as peers arrive. 
Depends on the seeder upload rate $U_s$, if the seeder upload small then minimum capacity of CDN proxy must be increased. 
For example $N=1000$ and $U_s=0.1$, in the peak of simulation, the minimum capacity of CDN proxy must be provided $U_p$ around $330$, while in case $U_s=0.8$, the minimum capacity of CDN proxy must be provided $U_p$ around $190$.
