...
Moreoverm we say that the minimum price $P^{(s)}_min$ that makes an ISP indifferrent between strategy $s \in \{s2\}$ and strategy $s_1$ can be found by identifying where the payoff of strategy $s$ equal the payoff of strategy $s_1$. 
...
\begin{equation}\label{eq:ispminprice2}
	N P^{(s)}_{min} - \tau^{(s)} - C^{(s)}_i + \lambda^{(s)}_c = N P^{(s_1)} - \tau^{(s1)}  
\end{equation}
...
\begin{equation}\label{eq:ispminprice3}
	P^{(s)}_{min} = P^{(s_1)} - \frac{1}{N} ( \tau^{(s1)} - \tau^{(s)} -  C^{(s)}_i \lambda^{(s)}_c )
\end{equation}
...
Above equation also express that ISP at maximum are willing to reduce its initial price $P^{(s_1)}$ by the expression $\frac{1}{N} ( \tau^{(s1)} - \tau^{(s)} -  C^{(s)}_i \lambda^{(s)}_c )$.
Moreover it can be interpreted as a combination of the reduction in ISP total costs plus its utility increase due to preferences for increased control.
...
\begin{itemize}
	\item If $\lambda^{(s)}_c \ge C^{(s)}_i$ the the maximum price reduction will be greater than of equal the reduction in traffic costs.
	\item If the investment cost for strategy $s$ approach the expected reduction in traffic cost, $C^{(s)}_i \to (\tau^{(s1)} - \tau^{(s)})$.
	\item As $\frac{1}{N} ( \tau^{(s1)} - \tau^{(s)} -  C^{(s)}_i \lambda^{(s)}_c )$ represents the maximum price reduction that an ISP might give, we can futhermore express how greedy an ISP is by introducing an parameter $\gamma$ that tells us what percentage of the maximum price reduction above an ISP is willing to share.  
\end{itemize}
Given observations above we can define the ISP price reduction $\theta^{(s)}$ when choosing strategy $s$ to be:
\begin{equation}
 \theta^{(s)} = \frac{\gamma}{N} ( \tau^{(s1)} - \tau^{(s)} -  C^{(s)}_i \lambda^{(s)}_c )
\end{equation}
...
\begin{equation}
	Q^{(s_1)} ( 1 + d^{(s)} ) - P^{(s)}_max = Q^{(s_1)} - P^{(s_1)}
\end{equation}
...
\begin{equation}\label{eq:min-max-interval}
	P^{(s_1)} - \frac{1}{N} ( \tau^{(s1)} - \tau^{(s)} -  C^{(s)}_i \lambda^{(s)}_c ) < P^{(s)} <  P^{(s_1)} + Q^{(s_1)} d^{(s)} 
\end{equation}
...
\begin{align}
	...
	P^{(s_2)} = P^{(s_1)} - \frac{\gamma}{N} (\tau^{(s_1)} - \tau^{(s_2)} - C^{(s_2)}_i - \lambda^{(s_2)}_c)
\end{align}
The strategy restrictions as follows:
\begin{itemize}
	\item Choose strategy $s=s_2$ if $ P^{(s_2)} + \frac{\gamma}{N} (\tau^{(s_1)} - \tau^{(s_2)} - C^{(s_2)}_i - \lambda^{(s_2)}_c) > P^{(s_1)}$
	\item Choose strategy $s=s_1$ if $ P^{(s_1)} > P^{(s_2)} + \frac{\gamma}{N} (\tau^{(s_1)} - \tau^{(s_2)} - C^{(s_2)}_i - \lambda^{(s_2)}_c)$ 
\end{itemize}
