...
\newtheorem{theorem8}{Definition}\label{la:isppricereduction}
...
\begin{equation}
  \tau^{(s_2)}	- C^{(s_2)_i} = \tau^{(s_1)}
\end{equation}
The strategy restrictions as follows:
\begin{itemize}
	\item Choose strategy $s=s_2$ if $ \tau^{(s_2)}	- C^{(s_2)_i} < \tau^{(s_1)} $
	\item Choose strategy $s=s_1$ if $ \tau^{(s_2)}	- C^{(s_2)_i} > \tau^{(s_1)} $ 
\end{itemize}
