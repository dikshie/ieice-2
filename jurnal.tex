...
Moreover, we define $RR\frac{n_l}{n_s}$, we can get:
\begin{equation}
	\frac{U_p}{n_s} = (1 + RR - min (u_s,RR))
\end{equation}
...
In order CDN proxy able to do that following simple admission policy can be used: 1) the system starts admitting new arrivals as seeders until $n_s.u_s < n_l.r$. 2) all arrivals after that point are admitted as leechers. 3) when a seeder leaves the system, CDN proxy can calculate $n_s.u_s < n_l.r$ again, depends on the result if $n_s.u_s < n_l.r$ then one of the leechers is randomly selected to become a seeder.
