...
\begin{equation}
	U^{(s)} = Q^{(s)} - P^{(s)}
\end{equation}
Where $Q^{(s)} > P^{(s)}$. and $Q$ is bandwidth. Later in next we normalize $Q$ to be same metrics with $P$
\end{theorem}
...
Futhermore, we can make observation that $\frac{1}{N} ( \tau^{(s_1)} - \tau^{(s)} -  C^{(s)}_i )$ represents the maximum price reduction that an ISP might give. 
...
\end{equation}
We found that the maximum value of the subscription free $P^{(s)}$ depends on the perceived performance $Q^{(s)}$.
...
\begin{eqnarray}
	...
	P^{(s_2)} &=& P^{(s_1)} - \frac{\gamma}{N} (\tau^{(s_)} - \tau^{(s_1)} - C^{(s_2)}_i )
\end{eqnarray}
