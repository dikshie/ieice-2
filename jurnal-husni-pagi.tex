...
We model this system as a P2P-CDN shown in Fig.\ref{fig:twotier2} for a single bitrate streaming, with a stochastic fluid model similar to \cite{4215694}.
While the authors in \cite{4215694} focus on probability of degraded service on small and large systems, our work focus on the minimum capacity of CDN proxy to support the system, as it is very important to ISP for capacity planning.
On next subsection we also present a model of simple game theory between ISP who operate P2P-CDN and users to find out the indifferent utility function between ISP and users.

\begin{figure}[tb] 
\begin{center}
\includegraphics[scale=0.35]{graphs/two-tier-cdn-topology-2.eps}
\end{center}
\caption{A P2P-CDN model for streaming with a single CDN proxy and several seeders and leechers. Leechers do not upload traffic to the P2P network to minimize the streaming latency.}
\label{fig:twotier2}
\vspace{-2mm}
\end{figure}
...
%For CDN proxy can fulfill required capacity following inequality must be satisfied: $n_s.u_s \geq n_l.r$.
...
\end{center}
\caption{System in stationary state. Each line has an inflexion point related to $u_s$, $u_s > 0$\.}
