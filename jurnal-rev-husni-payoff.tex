...
In this section, we present simple game theory approach to analyze ISP strategies.
...
Basic elements of a game are: rational players, strategies, and payoff or utility.  
Rational players choose a strategy for maximizing its payoff or utility. 
We use the extensive-form game, which is a representation of the corresponding decision nodes in a directed tree, in this analysis.
In our model, the ISP has already attempted to implement a peer-assisted CDN, and it is reasonable to assume that the ISP is the game initiator.  
We model the game as a Stackelberg game, also known as a leader-follower game, which is a class of game that concerns with situation where the leader initiates the decision making and where the follower responds to the actions by the leader.  
Moreover in our model, ISP can be modeled as the leader who decided to implement peer-assisted or not according to their expected payoff, while users can be modeled as the followers that evaluate the decision of the ISP and respond according to their available actions and payoffs.
In this section, we will focus on ISP side.  

\newtheorem{theorem}{Definition}
\begin{theorem}[ISP Strategies]
We denote the ISP strategies as:
\begin{itemize}
	\item $s_1 = (CDN, P^{s_1})$ : the ISP decides to use CDN only and thus keeps charging the initial subscription fee $P^{s_1}$;
	\item $s_2 = (CDNP2P, P^{s_2})$ : the ISP decides to offer peer-assisted CDN with subscription fee $P^{s_2} \le P^{s_1}$;
\end{itemize}
and the ISP strategy space is $S_{isp} = \{s_1,s_2\}$.
\end{theorem}
...
\end{center}
\caption{ISP strategies in the extensive form.}
\label{fig:gametree}
\vspace{-2mm}
\end{figure}
...
\begin{equation}
	\pi^{(s_1)}_{(N)},
\end{equation}
while for strategy $s_2$, it is:
\begin{equation}
	\pi^{(s_2)}_{(N)} = \pi^{(s_1)}_{(n_l | n_s)} + \pi^{(s_2)}_{(n_s)}.
\end{equation}
Therefore the payoff difference between strategy $s_1$ and $s_2$ is:
\begin{eqnarray}
